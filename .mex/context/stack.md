...
- *fastembed* — alternative local embedder (`optionalDependencies`), in-process ONNX via onnxruntime-node on the CPU — no separate service, no GPU. Model: *bge-base-en-v1.5* (768 dims, matches the fixed sqlite-vec schema). Selected with `AGENT_SEARCH_EMBED_PROVIDER=fastembed`. The package is dynamic-`import()`ed and given an actionable "npm install fastembed" error if missing. fastembed's `passageEmbed`/`queryEmbed` apply the model's retrieval prefixes, so the fastembed embedder does NOT add the nomic `search_document:`/`search_query:` prefixes the ollama one does.
- *@huggingface/transformers* (transformers.js) — third local embedder (`optionalDependencies`), in-process with `device:'webgpu'` for GPU acceleration. Model: *Xenova/bge-base-en-v1.5* (768 dims), CLS pooling + the bge query instruction on queries only. WebGPU needs a WebGPU runtime (browser/Electron); plain Node has no `navigator.gpu`, so it falls back to CPU with a one-time warning. Selected with `AGENT_SEARCH_EMBED_PROVIDER=transformers`. Supports a local-only mode (`env.allowRemoteModels=false` + a model dir) for air-gapped use. Stored model id is the lowercased HF id (`xenova/bge-base-en-v1.5`), distinct from fastembed's `bge-base-en-v1.5`, so the guard forces a reindex when switching even though both are bge.
- `createEmbedder()` (`src/embed/factory.ts`) is the ONLY place that picks a provider (via `AGENT_SEARCH_EMBED_PROVIDER`); everything else depends only on the `Embedder` interface.
